...
- `${name}` 如果类型是数组（ `String[]` , `Integer[]` , ...）或集合（ `Set` , `List` , ...），则先展开（逗号分割），再进行 SQL 片段的替换；
- `${!name}` 名字前多了前缀符号( `!` )，如果类型是数组（`String[]` , `Integer[]` , ...）或集合（ `Set` , `List` , ...），则先展开（逗号分隔），并做一定的字符串安全处理，再进行 SQL 片段的替换。
